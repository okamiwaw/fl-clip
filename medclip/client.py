...
class Client:
    ...
    def local_train(self):
        ...
        optimizer = optim.Adam(loss_model.parameters(), lr=self.textvision_lr, weight_decay=self.weight_decay)
    ...
    def select_train(self):
        ...
        optimizer = optim.Adam(self.select_model.parameters(), lr=self.select_lr, weight_decay=self.weight_decay)
        progress_bar = tqdm(enumerate(self.train_loader), total=len(self.train_loader), leave=True)
        scaler = GradScaler()
        for i, batch_data in progress_bar:
            optimizer.zero_grad()
